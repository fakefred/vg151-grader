...
class GitWorker:
    ...
    def checkIndvProcess(self, groupNum, hwNum):
        ...
        if not os.path.exists(repoDir):
            repo = git.Repo.clone_from(
                f"ssh://git@focs.ji.sjtu.edu.cn:2222/ENGR151-22/{repoName}.git",
                repoDir,
                branch="master",
            )
        else:
        ...
        for stuID, stuName in self.hgroups[repoName]:
            try:
                if f"origin/{stuID}" not in remoteBranches:
                    ...
                    scores[stuName]["indvComment"].append("Individual branch missing")
                    continue
                ...
                if not os.path.exists(hwDir):
                    ...
                    scores[stuName]["indvComment"].append(
                        f"Individual branch is missing h{hwNum} dir"
                    )
                else:
                    for fn, path in [
                        ...
                        if fn in self.mandatoryFiles:
                            ...
                            scores[stuName]["indvFailSubmit"] = 1
                            scores[stuName]["indvComment"].append(
                                f"Individual branch is missing h{hwNum}/{fn} file"
                            )
                ...
                dirList = list(
                    filter(
                        lambda x: x
                        not in [".gitignore", ".git", *[f"h{n}" for n in range(20)]]
                ...
                if dirList:
                    ...
                    scores[stuName]["indvComment"].append(
                        f"Individual branch has redundant files: {', '.join(dirList)}"
                    )
                if os.path.exists(hwDir):
                    dirList = os.listdir(hwDir)
                    dirList = list(
                        filter(
                            lambda x: x not in self.mandatoryFiles
                            and x not in self.optionalFiles
                            and not GitWorker.isREADME(x),
                            dirList,
                        )
                    )
    ...
    def checkGroupProcess(self, groupNum, hwNum):
        ...
        if f"h{hwNum}" not in tagNames:
            ...
            for _, stuName in self.hgroups[repoName]:
                ...
                scores[stuName]["groupComment"].append(f"tags/h{hwNum} missing")
            return scores
        ...
        if os.path.exists(os.path.join(repoDir, ".gitea")):
            dirList.extend(
                [
                    fn
                    for fn in os.listdir(os.path.join(repoDir, ".gitea"))
        ...
        dirList = list(
            filter(
                lambda x: x
                not in [".gitignore", ".git", ".gitea", *[f"h{n}" for n in range(20)]]
                and not GitWorker.isREADME(x),
                dirList,
            )
        )
        if dirList:
            ...
            for _, stuName in self.hgroups[repoName]:
                ...
                scores[stuName]["groupComment"].append(
                    f"tags/h{hwNum} has redundant files: {', '.join(dirList)}"
                )
        if os.path.exists(hwDir):
            dirList = os.listdir(hwDir)
            dirList = list(
    ...
    def checkProjProcess(self, id_, name, projNum, milestoneNum):
        ...
        if not os.path.exists(repoDir):
            repo = git.Repo.clone_from(
                f"ssh://git@focs.ji.sjtu.edu.cn:2222/ENGR151-22/{repoName}.git", repoDir
            )
        else:
        ...
        if milestoneNum:
            ...
            self.logger.debug(f"{repoName} checkout to tags/m{milestoneNum} succeed")
            ...
            if projNum == 1:
                ...
                for fn in getAllFiles(repoDir):
                    if (fn.endswith(".c") or fn.endswith(".h")) and not passCodeQuality(
                        os.path.join(repoDir, fn), language[projNum - 1]
                    ):
                        self.logger.warning(f"{repoName} {fn} low quality")
                        scores[stuName]["projComment"].append(f"{fn} low quality")
        ...
        if not os.path.exists(repoDir):
            ...
            if "origin/master" not in remoteBranches:
                ...
                for stuInfo in students:
                    scores[stuInfo[1]]["projComment"].append(f"master branch missing")
                return scores
            repo.git.reset("--hard", "origin/master")
            repo.git.clean("-d", "-f", "-x")
        if milestoneNum:
            ...
            if not list(filter(GitWorker.isREADME, os.listdir(repoDir))):
                self.logger.warning(f"{repoName} README file missing")
                for stuInfo in students:
                    scores[stuInfo[1]]["projComment"].append(f"README file missing")
            for fn in getAllFiles(repoDir):
                if (
                    any(
                        (
                    ...
                    self.logger.warning(f"{repoName} {fn} low quality")
                    for stuInfo in students:
                        scores[stuInfo[1]]["projComment"].append(f"{fn} low quality")
        else:
            self.logger.debug(f"{repoName} pull succeed")
        return scores
    ...
    def checkIndv(self):
        ...
        if self.args.rejudge < 0:
            with multiprocessing.Pool(self.processCount) as p:
                res = p.starmap(
                    self.checkIndvProcess,
                    [(i, hwNum) for i in range(1, len(self.hgroups.keys()) + 1)],
                )
        else:
    ...
    def checkProj(self, projNum, milestoneNum):
        ...
        if projNum in [1, 2]:
            infos = [
                [*info, projNum, milestoneNum]
                for hgroup in self.hgroups.values()
                for info in hgroup
            ]
            with multiprocessing.Pool(self.processCount) as p:
