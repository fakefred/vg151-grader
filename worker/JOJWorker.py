...
class JOJWorker:
    ...
    def uploadZip(self, homeworkID, problemID, zipPath, lang):
        ...
        csrfToken = soup.select(
            "#panel > div.main > div.row > div.medium-9.columns"
            " > div.section > div.section__body > form"
            " > div:nth-child(3) > div.columns > input[type=hidden]:nth-child(1)"
        )[0].get("value")
    ...
    def getProblemResult(self, homeworkID, problemID, zipPath, lang):
        ...
        self.logger.debug(f"{zipPath} upload succeed, url {response.url}, result {res}")
        return res
    ...
    def checkGroupJOJProcess(self, groupNum, hwNum, jojInfo, fns, problemID):
        ...
        res = self.getProblemResult(
            jojInfo["homeworkID"], problemID, zipPath, jojInfo["lang"]
        )
        # os.remove(zipPath)
        return res
    ...
    def checkGroupJOJ(self, jojInfo):
        res = {}
        hwNum = self.args.hw
        for i, (key, value) in enumerate(self.hgroups.items(), start=1):
            ...
            jojFailExercise = min(
                sum(
                    [int(acCount < 0.25 * totalCount) for acCount, totalCount in scores]
                ),
                2,
            )
            jojFailHomework = int(
                sum([item[0] for item in scores])
                < 0.5 * sum([item[1] for item in scores])
            )
    ...
    def checkProjJOJ(self, jojInfo):
        ...
        homeworkID, problemID = (
            jojInfo[projNum]["homeworkID"],
            jojInfo[projNum]["problemID"],
        )
        infos = [
        ...
        for id_, name, projNum, milestoneNum in infos:
            ...
            comments = [self.checkProjJOJProcess(repoName, homeworkID, problemID)]
...
if __name__ == "__main__":
    ...
    res = JOJWorker(JOJ_COURSE_ID, SID).getProblemResult(
        "5f66161a91df0600062ff7aa", "5f6614eb91df0600062ff7a7", "ex2.zip", "matlab"
    )
    print(res)
